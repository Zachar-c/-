...
2. 待办（2026-09-06 合拢后）：①CT 契约回写——`school_name`/`school_label` 快照键补进 `docs/contracts/2026-09-02-domain-ui-contract.md`（✅ a4e701b）；②RUI 退役收官（✅ 2fcf22e）——`ui_masters/wenzhen_{battle,map}_master` 与 `ui/screens/{battle,map}_screen.guitkx` 生成层退役（全屏官方 .tscn，`MASTER_SCENE_PATHS` 即唯一路由表）；**组件级 `.guitkx`（ui/widgets/*）保留**——`guitkx_codegen.compile_all` 遍历全库且生成物 `.gd` 仍被 acceptance_driver/preview 引用，删源码会破坏生成链；③**D1b 跨流派合炼矩阵**（主计划 Phase D 专项工单，2026-09-06 裁定开）——打通 3-5 转合炼链（现仅 2 条配方产出 rank3+），输入=逐族跨流派方向策展；缺目录蛊 8 项登记审计清册 §6.1（审计清册二阶段）。
