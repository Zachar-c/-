...
1. **World Model Stage 0：蛊界世界规则基准审计（2026-09-16 用户裁定：最高优先级）**：以 `docs/superpowers/specs/2026-09-16-wenzhen-world-model-correction-design.md` 为最高设计宪章。
   ...
   - **仅剩缺口 2 待裁定**：现役领域没有「未炼化 → 已炼化」的命令（`refined_instances()` 状态白名单只有 refined/contracted/weakened）。落地需先裁定状态模型与代价口径（真元/念头/魂魄），涉及 `resolver.gd` 等 Shared 单写者区，须走 Agent Ownership 五步协议 + 契约回写。**通过前禁止扩大蛊目录与流派数**；身份与择道改造仍待裁定。
