...
- 中：音效系统接入（Kenney.nl 开源音效已在 `assets/audio/`，经 `AudioManager` 路由——SFX 已接线：battle_screen_view 5 处 + 按钮 ui_click 全局，wenzhen_master_theme 统一触发）。**音频双系统收敛（2026-09-06 侦察）**：正交分工成立——AudioManager（autoload，SFX 注册表+8 player 池）管短音效；AudioDirector（main.tscn 节点）管 BGM（run_controller 视图切换 play_bgm 已接线）；主音量经 `run_controller._apply_master_volume` 设 **AudioServer master bus**（一杆到底覆盖两系统，无需逐系统接线）。无需合并。NPC 立绘开源化（OpenGameArt 等替代 AI 生成）。
