...
| `notes/` | 唯一台账 `ledger.md` + 事实争议队列 `fact-disputes.csv` + 编辑说明；历史台账归档于 `archive/`（只读） | 权威台账 |
...
- 裁决：卷一战力、经济、信息、时间等裁决（DEC-001~）已冻结于 `notes/ledger.md` 第 1 节；全书核证项 FB-001~016 见 `notes/archive/full-book-audit-register.md`（只读），未关闭项并入 `notes/ledger.md` 第 7 节。
