...
- 规范：`AGENTS.md` 为唯一完整规范源（开工前必读）；卷级裁决入 `notes/ledger.md` 第 1 节（冻结裁决与勘误）；历史卷级台账在 `notes/archive/`。
- 台账：唯一活台账 `notes/ledger.md`；事实争议队列 `notes/fact-disputes.csv`；历史台账在 `notes/archive/`。
