...
### 1.6 节点 UI 域全集（休整 / 商店 / 险地等带 visit 门禁节点）

- 任何被领域层 `rest_choice_required` / `shop_required` / `risk_required` 等门禁拒绝离开的节点，其 UI **必须**把领域命令全集暴露成可交互选项；选项不得被快照遗漏，也不得被 UI "代为判断"为不可达后整组隐藏。
- 当领域全集在当前状态下全部不合法（`disabled=true`）时，UI 必须额外暴露**带确认的显式退出命令**（休整节点 = `rest mode=skip`），与领域一致的 `requires_confirm` 文案展示，且事件日志落 `rest_skipped`（或对应节点 `xxx_skipped`）；不得让"全禁用 + 离开被门禁"成为软锁。
- 节点屏接受确认后的事件日志 reason 必须与领域 `_event(..., reason, ...)` 同源；UI 不发明 reason。
