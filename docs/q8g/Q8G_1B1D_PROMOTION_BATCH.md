...
> - **状态**：✅ **CLOSED（2026-09-13 审阅正式关闭）**——红线命中与调整已归档；全量 unit 1418/1418、交互闭环 17 屏全绿
> - **📎 sword 累计成本审计拆解（审阅要求补录）**：红线比较口径 = **累计成本（石 + 材料 value）vs 直取价 70**。sword 链 4 步：石 6/8/12/18（合计 **44**）+ 材料 value 1×2 / 2×2 / 3×2 / 5×2（合计 **22**）→ 逐步成本 8/20/38/28，逐步累计 **8 / 20 / 38 / 66**。即 **66 = 44 石 + 22 材料 value**，66 < 70 ✓。违例前的标准阶梯 125 = 103 石 + 22 材料，同一材料口径。逐步拆解已同步写入 4 条 sword recipe 的 `source` 字段，供 F8/Batch 2 直接追溯
...
| sword | 鞘蛊 → 藏锋蛊 → 剑锋蛊 → 剑气蛊 → 飞剑蛊 | 第1–2步 def→atk→mov（鞘→藏锋→剑锋→剑气→飞剑） | 断刃屑 → 陨铁屑 → 寒铁屑 → 天外玄铁屑 | **6/8/12/18**（累计 66 = 44 石 + 22 材料 value < 直取 70）|
