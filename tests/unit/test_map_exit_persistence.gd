...
func test_map_leave_confirm_save_then_hall_continue_restores_same_run() -> void:
	...
	assert_eq(controller.current_view_name(), "Title", "continue opens the school-pick subview")
	assert_eq(str(controller._hall_subview), "schools")
	hall_commands = controller._build_commands("Title")
	hall_commands["select_school"].call("blood")
	hall_commands = controller._build_commands("Title")
	hall_commands["load_run"].call()
