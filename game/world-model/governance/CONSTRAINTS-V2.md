...
> **2026-09-20 A 清理**：world-model 的 Python 运行时、镜像数据与验收脚本已删除；本文件中的旧命令只作历史记录，
> 当前可执行验收以 `game/tools/test.ps1` 与 `game/tools/check.ps1` 为准。
...
| # | 规则 | 落地方式（可执行） |
| --- | --- | --- |
| **R1** | **数据即规范**：结构约束由运行时校验与测试强制，不靠文档说服人 | `game/tests/unit/test_data_driven_guard.gd`、`ContentCatalog.validate` |
| **R2** | **一条命令验收**：GUT + 启动探针 + 契约漂移 + diff 检查 | `game/tools/check.ps1` |
| **R3** | **单一裁定入口**：一条裁定一个文件，替代批次文档堆 | `rulings/*.json`（见 §4） |
| **R4** | **可回滚**：任何文件都可改；改前检查 `git diff`，改后运行验收 | Git + `game/tools/test.ps1` |
| **R5** | **约束必须可执行**：任何新规则必须写成测试/脚本里的断言；写不进脚本的规则不许存在 | GUT 断言 |
| **R6** | **验收不靠人勾选**：结论由脚本输出，不靠人读文档打勾 | `game/tools/check.ps1` |
...
> **正确口径**：world-model Python 工具链已于 2026-09-20 删除；现役验收只看仓库脚本的真实输出文本
> （GUT 的 `Passing` / `Failing` / `Asserts` 三项数字），不单独把退出码当作充分证据。
...
| 目的 | 旧做法 | 新做法 | 成本 |
| --- | --- | --- | --- |
| 改一个数值 | 5 步协议 + 3 登记册 + Gate | 直接改 + 跑对应 GUT 测试 | 分钟级 → 秒级 |
| 加一个实体 | 写 spec + 登记 source_ids + 审查 | 加数据 + `ContentCatalog.validate` 通过 | 天级 → 分钟级 |
| 判断能否提交 | 17 审计标签 + 约 30 个 verify 脚本 | `game/tools/check.ps1` 并核对真实输出 | 小时级 → 秒级 |
| 回滚 | 手动 + git 纪律 | Git diff / 历史提交 | 分钟级 → 秒级 |

## 6. 本文件不做什么

- 不把已删除的 world-model Python 运行时重新当作现役依赖；需要时从 Git 历史恢复并重新立项。
- 不主张作者身份或版权。
