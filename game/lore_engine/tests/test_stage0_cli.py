...
class Stage0CliTests(unittest.TestCase):
    ...
    def test_committed_canonical_reports_are_self_consistent(self) -> None:
        canonical_dir = ROOT / "docs/lore/generated"
        baseline = json.loads(
            (canonical_dir / "world-model-baseline-v1.json").read_text(encoding="utf-8")
        )
        gate = (canonical_dir / "world-model-stage0-gate.md").read_text(encoding="utf-8")
        ...
        self.assertIn(
            f"- `world-model-baseline-v1.json`: `{_sha256(canonical_dir / 'world-model-baseline-v1.json')}`",
            gate,
        )
        self.assertIn(
            f"- `world-model-baseline-v1.md`: `{_sha256(canonical_dir / 'world-model-baseline-v1.md')}`",
            gate,
        )
