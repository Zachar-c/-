...
const mechanisms = {
  ...
  notCovered: [
    ...
    { name: '体印动作（take_imprint）', why: '效果写入 body_imprints（social_command_rules.gd:784-794 的铁骨体印）；本原型没有体印系统，登记不实现' },
    ...
    { name: '休息类节点的一次性门禁', why: 'rest / refinement / cultivation 属休息类：rest_rules.gd:39,55,92,115 规定一节点只能消费一次，未消费就离开会被 rest_choice_required 拒绝（social_command_rules.gd:586-589）；本原型没有节点内多次行动模型，该门禁搬不进来，refine / cultivate / rest 等专属动作因此一并不接' },
    { name: '其余节点类型的专属结算', why: 'contact / caravan / event / refinement / cultivation / ledger / inheritance / commission / pursuit / earth_vein / seclusion 等类型各有专属选项与命令面（商队、炼蛊、修行、总账、遗葬传承等），本原型固定图只放战斗与非战斗三类模板，其余类型未接入' },
