...
(() => {
  ...
  function header(){return `<h1>问真 · 快速玩法实验</h1><p class="muted small">独立实验版 · 选择猎物、炼出解法、重构杀招，再战 Boss　${btn('show-guide','查看本页指引',false,'class="secondary"')}${btn('sound',state.soundOn?'音效：开':'音效：关',false,'class="secondary"')}</p><div class="stats">${stat('气血',`${state.hp}/${state.maxHp}`)}${stat('真元',`${state.qi}/${state.maxQi}`)}${stat('元石',state.stones)}${stat('进度',`${state.stage}/3 战`)}${stat('已组杀招',`${state.moves.length} 式`)}${state.phase==='battle'?stat('本回合念头',`${state.battle.thoughts}/3`):''}</div>`;}
  ...
  function guideHtml(){
    ...
    const tips={
      ...
      battle0:['先计划，再让敌人出手','每回合有 3 点念头。侦查、攻击、防御各花 1 点，可按需要排序；点“结束回合”后敌人才按预告出手。每回合随后恢复 2 真元。战斗只能使用本场上阵的蛊及对应杀招。'],
  ...
  function startHtml(){return `<section class="panel"><h2>一局的目标</h2><p class="lead">每战选择敌人，获取专属材料与蛊。起手的月光蛊伤害更高，针影蛊命中更稳；炼蛊还能打开破甲、追风或封脉分支。战前选择两只攻击蛊、一只辅助蛊，本场只能使用它们及对应杀招。赢过两场普通战，再击败三劫守关人。</p><p>每回合有 3 次行动：可以先侦查、再攻击、最后防御；点“结束回合”后敌人才按预告出手。战败即终局。</p>${btn('start','开始新局')}</section>`;}
  function routeHtml(){const ids=ROUTES[state.stage]||[];return `<section class="panel"><h2>第 ${state.stage+1} 战 · 选择猎物</h2><p>先看自己缺哪种材料；每种敌人掉落对应的炼蛊钥匙。</p><div class="cards">${ids.map(id=>{const e=ENEMIES[id];return `<article class="card enemy"><h3>${e.name}</h3><p>${e.problem}</p><p>胜利：${e.stones} 元石 · ${M[e.drop]} · ${G[e.gu].name}</p>${btn('fight','挑战',false,`data-id="${id}"`)}</article>`;}).join('')}</div></section>`;}
  ...
  function battleHtml(){
    ...
    const timeline=b.timeline?.length?`<div class="turn-timeline"><strong>近期行动</strong><ol>${b.timeline.map(x=>`<li>${esc(x)}</li>`).join('')}</ol></div>`:'';
    return `<section class="panel enemy"><h2>${e.name} · 第 ${b.turn} 回合</h2><p>${e.problem}</p><p class="intent"><strong>敌方意图：</strong>${intent.label}，结束回合时造成 ${intent.damage} 伤害；你有 ${b.shield} 护盾，预计失血 ${Math.max(0,intent.damage-b.shield)}。</p><div class="arena"><div class="fighter player ${b.last?.playerLoss?'hit':b.last?.blocked?'blocked':''}"><div class="face" aria-hidden="true">◈</div><div class="name">蛊师 · 你</div><div class="number">${state.hp}/${state.maxHp} 气血</div>${meter(state.hp,state.maxHp)}<div>真元 ${state.qi}/${state.maxQi} · 护盾 ${b.shield}</div></div><div class="versus">对峙</div><div class="fighter foe ${b.last?.enemyLoss?'hit':b.last?.missed?'blocked':''}"><div class="face" aria-hidden="true">◇</div><div class="name">${e.name}</div><div class="number">${b.hp}/${e.hp} 气血</div>${meter(b.hp,e.hp)}<div>当前：${currentProblem()}</div></div></div>${feedbackHtml(b)}${timeline}<div class="stats">${stat('本回合念头',`${b.thoughts}/3`)}${stat('敌人回合伤害',`${intent.damage}，结束回合后结算`)}${stat('侦查',b.exposed?'下一击看穿闪避/反制':'未侦查')}${stat('护盾',b.shield)}</div><p class="notice">先安排最多 3 个行动，再结束回合。敌人不会在每次点击后出手。</p><h3>基础与侦查</h3><div class="row">${btn('basic','拳脚 · 2 伤害',!canAct)}${btn('inspect','侦查 · 看穿下一击，护盾 +1',!canAct)}</div><h3>本场上阵的战斗蛊</h3><div class="row">${guActions}</div><h3>本场辅助蛊</h3><div class="row">${supportActions||'<span class="muted">未上阵辅助蛊</span>'}</div><h3>可用杀招</h3><div class="row">${moveActions||'<span class="muted">本场没有可用杀招</span>'}</div>${btn('end-turn',canAct?'结束回合 · 让敌人出手':'念头耗尽 · 结束回合',false,'class="secondary"')}</section>`;
  }
