...
(() => {
  ...
  function battleHtml(){
    ...
    return `<section class="panel enemy"><div class="battle-kicker">遭遇 · ${state.stage+1}/3</div><h2>${e.name} · 第 ${b.turn} 回合</h2><p>${e.problem}</p>${arenaHtml(b,e)}<p class="intent"><strong>敌方意图：</strong>${intent.label}，结束回合时造成 ${intent.damage} 伤害；你有 ${b.shield} 护盾，预计失血 ${Math.max(0,intent.damage-b.shield)}。</p>${feedbackHtml(b)}${timeline}<div class="stats">${stat('本回合念头',`${b.thoughts}/3`)}${stat('敌人回合伤害',`${intent.damage}，结束回合后结算`)}${stat('侦查',b.exposed?'下一击看穿闪避/反制':'未侦查')}${stat('护盾',b.shield)}</div><p class="notice">先安排最多 3 个行动，再结束回合。敌人不会在每次点击后出手。</p><h3>基础与侦查</h3><div class="row">${btn('basic','拳脚 · 2 伤害',!canAct)}${btn('inspect','侦查 · 看穿下一击，护盾 +1',!canAct)}</div><h3>本场上阵的战斗蛊</h3><div class="row">${guActions}</div><h3>本场辅助蛊</h3><div class="row">${supportActions||'<span class="muted">未上阵辅助蛊</span>'}</div><h3>可用杀招</h3><div class="row">${moveActions||'<span class="muted">本场没有可用杀招</span>'}</div>${btn('end-turn',canAct?'结束回合 · 让敌人出手':'念头耗尽 · 结束回合',false,'class="secondary"')}</section>`;
  }
