...
| 表 | 内容 | 关键数值 |
|---|---|---|
| gu.json | 802 只蛊（220/157/180/97/147 按转） | value 锚 3/5/8/12/20；v1_effect 仅 48 只 |
| refinement_recipes.json | 392 配方 + 4 商队报价（**无 recipes.json，别找错**） | advance 6/10 石；free_mix 三结局 |
| enemies.json | 32 敌（common 13/elite 12/boss 7） | HP 3-20；意图五类；boss 50% 血转阶段 |
| shops.json | 33 offers | 石壳蛊 6 … 月华露 80；剑道系列 8→70 |
...
| contracts.json | 6 契约 | 血契打击 +30%；调试契约开局 1000 石 |
...
| npcs.json / events.json / relics.json / inheritances.json | 5 NPC / 2 事件 / 2 遗物 / 3 传承 | 均为小体量内容 |
...
原 SQL 数据排除令已解除（2026-09-10）：enemies.json 30 条由 validate 全量 + 池契约测试守卫[^1]。补一流派需改 6 处（schools/school_pools/gu.json 40 只/SCHOOL_IDS/测试硬编码/选择屏）[^4]。
