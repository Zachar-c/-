...
> ⚠️ **状态同步（2026-09-16 全量待办清扫）**：本文是 **基线 `b5d1857` 时点的历史快照**，正文保持原样。
> 后续已落地：**T15 刻痕通道**、**T16 残锋降转**（见 `specs/2026-09-12-sword-p2-t15-t16-spec.md` §5）、
> **refine 卡隐藏元石/材料成本已修**（§7-⑥ 关闭）。§7-③ logistics `bound` 死数据已在数据层关闭
> （`default_effect_by_role.logistics → heal:1`，`gu.json` 无显式 bound）。当前活待办以 `AGENTS.md` 为准。
