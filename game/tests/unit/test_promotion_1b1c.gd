...
# Q8-G Batch 1-B1-c (2026-09-13): blood / dream / luck / qi promotion
# chains - second content batch, single cross-role steps with documented
# lineage semantics (worksheet §6) - blood on a derived_D2 basis, the rest
# design_only. Same gates as 1-B1-a/b: M1 cross-definition, strict +1, same
# school, provisional quality ladder pinned against drift, and the M3 economic
# red line in SKIP-ledger form (all sixteen targets currently lack offers).
...
const CHAINS := {
	"blood": [
		["blood_droplet_gu", "blood_rec_2_24_gu"],
		["blood_rec_2_24_gu", "blood_atk_3_03_gu"],
		["blood_atk_3_03_gu", "blood_atk_4_10_gu"],
		["blood_atk_4_10_gu", "blood_atk_5_02_gu"],
	],
	"dream": [
		["dream_atk_1_09_gu", "dream_rec_2_34_gu"],
		["dream_rec_2_34_gu", "dream_atk_3_16_gu"],
		["dream_atk_3_16_gu", "dream_def_4_24_gu"],
		["dream_def_4_24_gu", "dream_atk_5_02_gu"],
	],
	"luck": [
		["luck_atk_1_08_gu", "luck_rec_2_28_gu"],
		["luck_rec_2_28_gu", "luck_atk_3_37_gu"],
		["luck_atk_3_37_gu", "luck_atk_4_38_gu"],
		["luck_atk_4_38_gu", "luck_atk_5_01_gu"],
	],
	"qi": [
		["qi_atk_1_04_gu", "qi_heal_2_20_gu"],
		["qi_heal_2_20_gu", "qi_atk_3_37_gu"],
		["qi_atk_3_37_gu", "qi_atk_4_38_gu"],
		["qi_atk_4_38_gu", "qi_atk_5_05_gu"],
	],
}
...
func test_fire_chain_runs_end_to_end_from_rank_one_to_rank_five() -> void:
	var run := _run_with_instances([{"definition_id": "blood_droplet_gu", "rank": 1}])
	run.stone = 500
	for band in [1, 2, 3, 4]:
		run.materials["mat_blood_%d" % band] = 4

	var current := run
	for i in CHAINS["blood"].size():
		var input_gu: String = CHAINS["blood"][i][0]
		var output_gu: String = CHAINS["blood"][i][1]
		var recipe_id := "promote_%s_to_%s" % [input_gu.trim_suffix("_gu"), output_gu.trim_suffix("_gu")]
...
func test_promotion_rejects_without_the_band_material() -> void:
	...
	var result := ResolverScript.apply(run, {
		"type": "refine_gu", "recipe_id": "promote_blood_droplet_to_blood_rec_2_24",
	}, catalog())
