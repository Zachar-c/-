...
**协议**：回填批＋系统补强批落地后，按 wiki 惯例做知识完整性回归。原题库文本不在库内（L0 外部提供），题面由审计明细主题重建（题意对齐、措辞不保证逐字一致）；三路独立答题代理（50＋25＋25）Wiki-Only 盲测，白名单＝lore/wiki 知识页（禁读原文/笔记/tools），状态口径与首测相同（命中＝页面有完整依据、部分＝答出一部分、无据＝页面无依据）。

| 批次 | 命中 | 部分 | 无据 | 触达率 |
|---|---|---|---|---|
| 首测（2026-09-25 审计） | 36 | 41 | 23 | 77/100 |
| **复测（2026-09-26，回填批＋系统补强批后）** | **75** | **20** | **5** | **95/100** |

分半：1–50 题＝37 命中/10 部分/3 无据（首测 21/19/10）；51–100 题＝38 命中/10 部分/2 无据（首测 15/22/13）。

**剩余无据 5 题**（全部为"原文确无"或题面前提待核，与回填批结论一致，非蒸馏缺口）：Q37 流派形成四条件（成文框架不存在）、Q49 一心二用破大转数（无此规则）、Q76 战役规模排名（无叙事表述）、Q94 雪民/人祖之心相碰撞（人祖传无此章）、Q44 升仙成功率统计数字（原文无统计）。

**残余部分 20 题**：均为"半有据"——缺失半边多系原文本身留白或题面超纲（如 Q55 红莲损宿命蛊所用蛊虫、Q63 无极护道人、Q68 龙公形态谱系细节、Q84 命运歌核心仙蛊、Q29 十绝体全名单〔后记仅八名带"等"〕、Q35 人祖十子全名单〔正文可考七名〕）。

**归一与局限**：①Q44 代理引 tools/benchmark-100q.md 定性句判"部分"，按知识页白名单口径归一为"无据"（277320 定性锚未入知识页，系回填批有意决定）；②状态由代理自判、编辑抽验未见反例，非双人交叉复核；③题面重建与首测原题存在措辞差，横向对比看分布、不逐题精确对齐；④两批回填为同一编辑线执行，存在"编辑自证"偏差风险——独立代理盲测可部分对冲，GEN 化裁决仍归 L0/L1。
