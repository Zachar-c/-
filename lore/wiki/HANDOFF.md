...
《蛊真人》知识库（lore/wiki，Narrative Compiler v0.1）已完成：Schema v2.1 **转正**（L0 批准）、三验证簇 + 规则精蒸馏七页 + 哲学轴五批（A 线）、B 线粗蒸馏八簇、C 线三弧（基准 50/50·49.5·49.5 在案）；**58 笔提交未推送**；门禁全绿。
...
- **C（剧情顺序）**：WTC ✅ 50/50 → CAR ✅ 49.5/50 → TKF 标段二（三王福地犬王传承，节 147 起）进行中。
...
- **C 线**：TKF 标段二 → benchmark-tkf。
- **跨线**：推送 58 笔（L0 发话）；`measure_distillation.py` 随窗口扩张重跑；canon-index 并档受 `game/` 冻结（L0 裁量）。
