...
1. **js/data.js 未提交**：工作树已重生成（含 B 线数据），但同批并行线有未提交的 gu.json/v1_battle.json/jbalance 等 WIP，为避免把他人改动烘进生成物而暂缓。**接手后**：等并行批提交后重跑 `node tools/build_data.mjs` 再单独提交 js/data.js。（2026-09-25 维护批复查：v1_battle.json 等并行 WIP 仍在工作树，继续暂缓。）
2. **bank-only 敌人**：bai_ice_warden、xiong_bear_handler、merchant_hall_inquisitor（以及既有 18 个）未接节点/pool；接入方式照 b4614cc 的三种 hook（分支节点/boss 池/route）。（2026-09-25 维护批：未动——数据接入与 js/data.js 重建/提交捆绑，等并行批收口后同批处理。）
3. **跨线冲突登记**（在 wiki 各页待核对）：
   - "百足家吞并黑家"（E 笔记）vs 黑楼兰黑家后世活动——✅ 已裁定（2026-09-25 维护批）：同一黑家（黑楼兰家族，大本营铁鹰福地），因黑楼兰被指涉八十八角真阳楼案遭北原围攻覆灭、被百足家吞并；黑楼兰当时随影无邪在外、后化名黑月，与"后世活动"不冲突。裁定详情见 roster-2.md《待核对》。
   - 虚道时代两说（图事成"近古产生" vs 原文 73156"上古盛行"）——归 A 线流派境界核验批；
   - 紫山真君名号双源（方源自称弟子 113996 ↔ 真身 236694）——✅ 已裁定（2026-09-25 维护批）：名号系方源随口编造、安在真实存在的紫发老乞丐（＝紫山真君本人）身上，方源杜撰时不知情（前世记忆无此人 148022、238258）；详见 roster.md《原著明确内容》。
4. ~~**G 笔记 3121xx 区间锚点系统性偏移**~~ ✅ 已清偿（2026-09-25 维护批）：B 页 6 处 G 锚点（309000–309158、312144–312182、312180–312182、312242–312258、312284–312320、312822–312838）回原文抽验全部实位相符——"系统性偏移"不成立，真实偏移仅限已修正的春秋蝉/爱情蛊两条（312128→313128、312140→313144）；结论登记于 roster.md《待核对》，后续引 G 笔记锚点仍须逐条回验。
