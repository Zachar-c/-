...
### L2 链式行质量标准（2026-09-26 结构化增密批验证有效，L0 裁定）

- 簇页 L2 事件行对重大战斗/事件序列须承载**完整因果链**（起因→关键转折→结果+代价），在行内单元格增密、一行一事件不拆行；纯数值/状态变化入实体页状态时间线；单点事实入簇页补录小节。
- 基准题集可测至该粒度；**口径校准（下调题标迁就页面）不作为达标手段**——页面达不到严格标准时做结构化增密（CPR 实证：严格 40.5 → 增密后 48.5）。
- 增密的边界：补的是「链上缺环」与实体级事实，不是转录原文叙事；页体积仍受簇页预算约束。
